...
# DST GAMES
    ...
    def get_dst_games
        # current_timeframe
        stats_resp = Faraday.get "https://api.fantasydata.net/api/nfl/fantasy/json/FantasyDefenseProjectionsByGame/#{@current_api_season}/#{@current_week}" do |req|
            req.params['key'] = ENV['FANTASY_DATA_KEY']
        end
        @stats_json = JSON.parse(stats_resp.body)
    end
    ...
    def create_dst_games
        ...
        tlfl_dsts.each do |tlfl_dst|
            dst_game = ProjectedTeamDstGame.find_by(team_dst_id: tlfl_dst.id, season: @current_season, season_type: @current_season_type, week: @current_week)
            if dst_stats = @stats_json.find {|fd_dst| fd_dst["PlayerID"] == tlfl_dst.fd_player_id}
                if dst_game
                    ...
                    tlfl_players = ProjectedPlayerGame.where(week: @current_week, nfl_team: tlfl_dst.nfl_abbrev)
                    ...
                    dst_game = ProjectedTeamDstGame.create(
